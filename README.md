...
**여러 시각화 화면을 하나의 끊기지 않는 사진 탐색 경험으로 연결합니다.** 장소·태그로 사진을 찾고, 상세에서 좋아요를 남기고, 모아본 뒤 이전 조건으로 돌아올 수 있습니다.

| 역량 | 실제 구현 |
| --- | --- |
| 상태 설계 | 공유 사진은 Zustand, 탐색 조건은 URL, 임시 조작은 local state. 새로고침·뒤로/앞으로 탐색 복원 |
| 재사용성 | 사진·앨범·좋아요에서 PhotoFeed/PhotoModal 재사용. 지도·피드의 좋아요 액션 공유 |
| 접근성·반응형 | 사진/앨범 키보드 조작, 모달 포커스 제한·Escape·복귀, 모바일 액션 가시성 |
| 최적화 | 피드 가상화·이미지 단계적 로딩 유지, 앨범 그룹 단일 순회, 불필요한 모달 이미지 로딩 방지 |
| 검증 | 브라우저 회귀 19개, 이미지·앨범·데이터/성능 계약 30개, TypeScript·production build |

체험: **장소로 찾기 → 검색/태그 → 사진 상세 보기 → 좋아요 → 좋아요 모아보기 → 브라우저 뒤로가기**. 앨범 탐색 조건도 URL로 복원됩니다.

[구현 결정·검증 방법·한계](docs/qa/frontend-completion-2026-09-12.md) · [디자인 기준](DESIGN.md)

최적화 항목은 구현 구조에 대한 설명입니다. 이번 변경의 LCP/FPS 개선율은 별도 측정하지 않았습니다.
## 목차
